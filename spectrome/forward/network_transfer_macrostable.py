...
#     cont_fdg_mat = loadmat("/data/rajlab1/user_data/parul/ad_fdg/CON_FDG_baseline.mat")
#     cont_fdg = cont_fdg_mat['CON_FDG_baseline'][:,2]
    
#     mine = np.amin(cont_fdg)
#     maxe = np.amax(cont_fdg)
#     a = 1 
#     b = 5

#     cont_fdg_scaled = cont_fdg

#     for j in range(K):
#         cont_fdg_scaled[j] = (b-a)*(cont_fdg[j]-mine)/(maxe-mine) + a;

#     cont_fdg = cont_fdg / np.linalg.norm(cont_fdg)

    for k in range(K):
        model_out += (frequency_response[k]) * np.outer(eigenvectors[:, k], np.conjugate(eigenvectors[:, k])) 
    
    # model_out = np.transpose(np.matmul(np.transpose(model_out),np.diag(cont_fdg_scaled)))
...
# #         model_out += frequency_response[k]*(eigenvectors[:,k]*Htotal + np.dot(np.outer(eigenvectors[:,k], np.conjugate(eigenvectors[:,k])), S))
