...
from spectrome.optim import sgmglobaloptim_FC, sgmglobaloptim_FC_pearson
...
#     brain.distance_matrix = get_mean_C(brain.distance_matrix)
    ...
    opt_res0 = dual_annealing(
        sgmglobaloptim_FC.global_corr,
        x0=inguess(0),
        bounds=bnds,
        args=(brain,F_ind_db,meg_ind_FC,rois_with_MEG,fvec),
        # maxiter=1000,
        maxiter=500,
        initial_temp=5230.0,
        seed=24,
    ...
    rcorr, spcorr = sgmglobaloptim_FC_pearson.global_corr(opt_res["x"], brain, F_ind_db, meg_ind_FC, rois_with_MEG, fvec)
...
if __name__ == '__main__':
    ...
    # pool.close()
    res2 = np.array(res)
    np.savetxt("/data/rajlab1/user_data/parul/spectromeP_results/results_globalSGM/alpha_experiments/microint_FC_blocks.csv", res2, delimiter=",",header="taue, taui, alpha, speed, gei, gii, tauC, r_tot, r_psd, r_sp, sub, flag, status, success")
