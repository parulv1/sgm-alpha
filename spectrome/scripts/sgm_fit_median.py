...
ind_conn_median = np.median(ind_conn,axis=2) #86X86

# ind_psd_xr = xr.open_dataarray(data_dir + '/individual_psd_reordered_smooth.nc')
ind_psd_xr = xr.open_dataarray('../data/individual_psd_reordered_matlab.nc')
ind_psd = ind_psd_xr.values

ind_psd_median = np.median(ind_psd,axis=2) # 68X40
...
brain.reducedConnectome = ind_conn_median
...
def optsgm(cdk,psd,rois_with_MEG,fvec,bnds):

    
    F_ind = psd # grab current subject's MEG
...
#     brain.distance_matrix = get_mean_C(brain.distance_matrix)
    ...
    rcorr, rcorr2, spcorr = sgmglobaloptim_pearson.global_corr(opt_res["x"], brain, F_ind_db, F_ind, rois_with_MEG, fvec)
...
def optsgm_st(cdk,psd,rois_with_MEG,fvec,mica_micro_intensity):
    
    res = optsgm(cdk,psd,rois_with_MEG,fvec,bnds1)
    ...
    if st>0:
        res = optsgm(cdk,psd,rois_with_MEG,fvec,bnds2)
    ...
    if st>0:
        res = optsgm(cdk,psd,rois_with_MEG,fvec,bnds3)
    ...
    if st>0:
        res = optsgm(cdk,psd,rois_with_MEG,fvec,bnds4)
...
if __name__ == '__main__':
    ...
    # print(res)
    res  = optsgm_st(ind_conn_median, ind_psd_median, rois_with_MEG, fvec, mica_micro_intensity)
    # pool.close()
    res2 = np.array(res)
